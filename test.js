...
container.style='position:static; display:flex;justify-conteny:center;align-content:center; text-align: center;top: 0px;left: 0px;width: 100%;height: 100%;padding: 2px 0px 0px;background: radial-gradient(circle, rgb(255, 75, 75) 0%, rgb(218, 63, 63) 100%)';
...
container.appendChild(divElement2);
    divElement2.style = `background: white; position: absolute; margin:auto ;display: grid;grid-template-columns: repeat(${number}, minmax(16px,auto));grid-template-rows: repeat(${number}, minmax(16px,auto));`
    for (let i = 0; i < number * number; i++) {
        const div = document.createElement('div');
        div.className = 'divElement'
        div.style = 'width:100%;border: 2px solid red; height: 100%;background: white'
